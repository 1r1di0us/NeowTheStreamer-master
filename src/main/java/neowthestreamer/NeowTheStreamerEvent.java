...
public class NeowTheStreamerEvent extends AbstractEvent {

    private static final Logger logger = LogManager.getLogger(NeowTheStreamerEvent.class.getName());

    private static final String ID = makeID("NeowTheStreamerEvent");
    ...
    private ArrayList<NeowTheStreamerReward> rewards = new ArrayList<>();
    ...
    public NeowTheStreamerEvent(boolean isDone) {
        waitingToSave = false;
        if (this.npc == null)
    ...
    public void update() {
        ...
        //for (NeowTheStreamerReward r : this.rewards)
        //    r.update();  //nothing to update
        ...
        if (waitingToSave && !AbstractDungeon.isScreenUp && AbstractDungeon.topLevelEffects.isEmpty() && AbstractDungeon.player
                .relicsDoneAnimating()) {
    ...
    protected void buttonEffect(int buttonPressed) {
        switch (this.screenNum) {
            case 0:
                dismissBubble();
                talk(TEXT[4]);
                if (this.bossCount != 0 || Settings.isTestingNeow || NeowTheStreamer.debugNeow) {
                    blessing();
                } else {
                    miniBlessing();
                }
            ...
            case 3:
                ...
                switch (buttonPressed) {
                    case 0:
                        ((NeowTheStreamerReward)this.rewards.get(0)).activate();
                        talk(TEXT[8]);
                        break;
                    case 1:
                        ((NeowTheStreamerReward)this.rewards.get(1)).activate();
                        talk(TEXT[8]);
                        break;
                    case 2:
                        ((NeowTheStreamerReward)this.rewards.get(2)).activate();
                        talk(TEXT[9]);
                        break;
                    case 3:
                        ((NeowTheStreamerReward)this.rewards.get(3)).activate();
                        talk(TEXT[9]);
                        break;
                }
                this.screenNum = 99;
    ...
    private void endlessBlight() {
        if (AbstractDungeon.player.hasBlight("DeadlyEnemies")) {
            ...
            AbstractDungeon.getCurrRoom().spawnBlightAndObtain(Settings.WIDTH / 2.0F, Settings.HEIGHT / 2.0F, (AbstractBlight)new Spear());
        }
        if (AbstractDungeon.player.hasBlight("ToughEnemies")) {
            ...
            AbstractDungeon.getCurrRoom().spawnBlightAndObtain(Settings.WIDTH / 2.0F, Settings.HEIGHT / 2.0F, (AbstractBlight)new Shield());
        }
        uniqueBlight();
    }
            ...
            if (temp != null) {
                ...
                if (temp != null) {
                    ...
                    if (temp != null) {
                        ...
                        AbstractDungeon.getCurrRoom().spawnBlightAndObtain(Settings.WIDTH / 2.0F, Settings.HEIGHT / 2.0F, (AbstractBlight)new GrotesqueTrophy());
                    }
                } else {
                    AbstractDungeon.getCurrRoom().spawnBlightAndObtain(Settings.WIDTH / 2.0F, Settings.HEIGHT / 2.0F, (AbstractBlight)new Muzzle());
                }
            } else {
                AbstractDungeon.getCurrRoom().spawnBlightAndObtain(Settings.WIDTH / 2.0F, Settings.HEIGHT / 2.0F, (AbstractBlight)new TimeMaze());
            }
        } else {
            AbstractDungeon.getCurrRoom().spawnBlightAndObtain(Settings.WIDTH / 2.0F, Settings.HEIGHT / 2.0F, (AbstractBlight)new MimicInfestation());
            return;
        }
    }
    ...
    private void miniBlessing() {
        ...
        this.rewards.add(new NeowTheStreamerReward(true));
        this.rewards.add(new NeowTheStreamerReward(false));
        this.roomEventText.clearRemainingOptions();
        this.roomEventText.updateDialogOption(0, ((NeowTheStreamerReward)this.rewards.get(0)).optionLabel);
        this.roomEventText.addDialogOption(((NeowTheStreamerReward)this.rewards.get(1)).optionLabel);
        this.screenNum = 3;
    }
    ...
    private void blessing() {
        ...
        dismissBubble();
        talk(TEXT[7]);
        this.rewards.add(new NeowTheStreamerReward(0));
        this.rewards.add(new NeowTheStreamerReward(1));
        this.rewards.add(new NeowTheStreamerReward(2));
        this.rewards.add(new NeowTheStreamerReward(3));
        this.roomEventText.clearRemainingOptions();
        this.roomEventText.updateDialogOption(0, ((NeowTheStreamerReward)this.rewards.get(0)).optionLabel);
        this.roomEventText.addDialogOption(((NeowTheStreamerReward)this.rewards.get(1)).optionLabel);
        this.roomEventText.addDialogOption(((NeowTheStreamerReward)this.rewards.get(2)).optionLabel);
        this.roomEventText.addDialogOption(((NeowTheStreamerReward)this.rewards.get(3)).optionLabel);
        this.screenNum = 3;
    }
